...
| `agentstatus avvisad: transportfel, IO-fel (öppna/läsa)` / `…, överflöde` | fw `agent-net` | the agent feed's real fetch outcome (OBS-12): `IO-fel` is the connection or read failing (the `torget-http` line beside it has the redacted target), `överflöde` a body over the cap. Before 2026-09-10 this line always said `ESP_FAIL` whatever the cause. |
