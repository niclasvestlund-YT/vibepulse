...
| `not_run` | Probe has not fired yet | Normally clears itself within seconds: the first-scan thread calls `get_limits()` before it scans, which starts the probe; the probe does not wait for the scan. If it persists, look for `förstaskanningen misslyckades` in the server log and `usageScanStatus` on `GET /`. 240 s (`LIMITS_EVERY_S`) is the gap between *completed* probes, not a wait for the first |
