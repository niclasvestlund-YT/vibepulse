...
if [ "$#" -lt 1 ] || [ "$#" -gt 2 ] || [ "$1" != "vibepulse" ]; then
  printf 'usage: %s vibepulse [waveshare_216|waveshare_241_v2|waveshare_18_v2]\n' "$0" >&2
  exit 2
fi
...
case "$board" in
  ...
  waveshare_18_v2) build_dir="$repo/sim/build-18-v2"; spec_dir="$repo/spec/boards/waveshare_18_v2" ;;
